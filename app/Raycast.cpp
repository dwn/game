...
void rrPretitleLoop() {
  ...
  if (appGetMouseDown()) { //Mouse interaction only, because some browsers require it in order to begin audio
    ...
    rrImageFetch("art/img/coast.jpg");
  }
  rrLoopUpdate();
}
...
void rrMainLoop() {
  ...
  if (inputCode&APP_INPUTQ&&readyForNextKey) {
    goWildCountdown=256;
    readyForNextKey=false;
  }
  ...
  //Update noise functions if "going wild"
  if (goWildCountdown) {
    noise1->update();
    agSetHueAngle(goWildCountdown<<3);
    goWildCountdown--;
  }
  //Drag to a stop
  if (velRgt)
